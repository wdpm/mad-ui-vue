...
function makeItRain(num) {
    ...
    // removes any old drops if exists
    while (elements.hasChildNodes()) {
        elements.removeChild(elements.lastChild);
    }

    for (let i = 0; i < num; i++) {
        let randomX = Math.floor(Math.random() * (pageWidth));//[0,pageWidth)
        let randomY = Math.floor(Math.random() * (pageHeight));//[0,pageHeight)
        let dropSpeed = Math.floor(Math.random() * (25 - 5)) + 5;// [5,25)
...
// add other button and angles to the rain.

// add a lightning for example. Also with Lightning class.